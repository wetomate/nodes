...
const validatorPath = path.resolve(
	__dirname,
	"validate-community-node-packages.js",
);
...
describe("community node package validation", () => {
	...
	it("reports dependency and registered-entry violations together", async () => {
		...
		assert.match(
			result.stderr,
			/must not declare runtime dependencies; found example/,
		);
		...
		assert.match(
			result.stderr,
			/source file nodes\/Missing\.node\.ts does not exist/,
		);
	});

	it("rejects unregistered node source files using the node suffix", async () => {
		const repositoryRoot = await createRepositoryFixture(
			{},
			{ unregisteredNodeSource: true },
		);

		const result = runValidator(repositoryRoot);

		assert.equal(result.status, 1);
		assert.match(result.stderr, /unregistered \.node\.ts source file/);
		assert.match(result.stderr, /nodes\/internal\/Internal\.node\.ts/);
	});
});
...
async function createRepositoryFixture(
	...
	const repositoryRoot = await mkdtemp(
		path.join(tmpdir(), "wetomate-validator-"),
	);
	...
	await writeFile(
		path.join(nodeRoot, "nodes/Example.node.ts"),
		"export class Example {}\n",
	);
	if (options.unregisteredNodeSource) {
		await mkdir(path.join(nodeRoot, "nodes/internal"), { recursive: true });
		await writeFile(
			path.join(nodeRoot, "nodes/internal/Internal.node.ts"),
